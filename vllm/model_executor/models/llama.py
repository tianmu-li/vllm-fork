...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if do_split and (seq_len*batch_size)//split_size>=2 and self.output_slice:
            ...
            num_slices_limit = (num_slices*25)//50
            for i,(hidden_state, residual) in enumerate(zip(hidden_states, residual_list)):
                ...
                if i==num_slices_limit:
                    output_list.append(tensor_model_parallel_all_reduce(torch.cat(output_list_partial[:num_slices_limit], dim=1)))
            ...
            residual_list_output.append(torch.cat(residual_list_partial[:num_slices_limit], dim=1))
            residual_list_output.append(torch.cat(residual_list_partial[num_slices_limit:], dim=1))
